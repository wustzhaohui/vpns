...
const Header: React.FC = () => {
      ...
      if (isAtTop) {
        ...
        headerContainerClasses =
          headerContainerClasses
            ...
            .trim() + ' shadow-none';
        if (isOnBlogPage && isAtTop) {
          // Re-apply border if removed by shadow logic
          headerContainerClasses += ' border-b border-gray-300';
        }
      } else {
  ...
  if (isOnBlogPage) {
    ...
    if (isAtTop && !headerContainerClasses.includes('border-b')) {
      // Ensure border if at top and somehow removed
      headerContainerClasses =
        headerContainerClasses
          .replace(/\bshadow-lg\b|\bshadow-none\b/g, '')
          .trim() + ' shadow-none border-b border-gray-300';
    } else if (isAtTop) {
      // Ensure shadow-none is present if border is also present
      if (!headerContainerClasses.includes('shadow-none')) {
        headerContainerClasses =
          headerContainerClasses.replace(/\bshadow-lg\b/g, '').trim() +
          ' shadow-none';
      }
    }
  }
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `text-[1.8rem] lg:text-[2rem] xl:text-[2.2rem] font-medium transition-colors duration-150 ${
                      isActive
                        ? useLightTextAndLogo && !isOnBlogPage // Blog page text is dark
                          ? 'text-white'
                          : 'text-brand-purple'
