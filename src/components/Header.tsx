...
const Header: React.FC = () => {
  ...
  const [isHeaderLanguageDropdownOpen, setIsHeaderLanguageDropdownOpen] = useState(false);
  ...
  const toggleHeaderLanguageDropdown = () => setIsHeaderLanguageDropdownOpen((prev) => !prev);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const applyHeroHeaderScrollBehavior = (isOnHomePage || isOnReferralPage) || (isMobileView && isOnLoginPage) ;
  ...
  }, [isOnLoginPage, isOnBlogPage, animationDuration, isMobileView, isOnHomePage, isOnReferralPage]);
  ...
  const useLightTextAndLogo = (isOnHomePage || isOnReferralPage) || (isMobileView && isOnLoginPage);
  
  let logoSrc = '';
  if (isOnBlogPage) {
    logoSrc = '/assets/logo_blog.png';
  } else {
    logoSrc = useLightTextAndLogo ? '/assets/logo.png' : '/assets/footer-logo.png';
  }


  let headerContainerClasses = `sticky top-0 z-50 h-[60px] md:h-[80px] transition-all duration-[${animationDuration}ms] ease-in-out`;
  let currentHeaderStyle: React.CSSProperties = {};

  if (isAtTop) {
    headerContainerClasses += ' shadow-none';
    if (isOnBlogPage) {
      headerContainerClasses += ' border-b border-gray-300';
    }
  } else {
  ...
  if (isMobileView) {
    if (isOnHomePage || isOnReferralPage) {
      if (isAtTop) {
        ...
        headerContainerClasses = headerContainerClasses.replace(/\bshadow-lg\b|\bshadow-none\b/g, '').trim() + ' shadow-none';
         if (isOnBlogPage && isAtTop) { // Re-apply border if removed by shadow logic
          headerContainerClasses += ' border-b border-gray-300';
        }
      } else { 
        currentHeaderStyle.background = 'linear-gradient(284deg, #cc5ce8 10%, #3150ed)';
        headerContainerClasses = headerContainerClasses.replace(/\bshadow-lg\b|\bshadow-none\b/g, '').trim() + ' shadow-[0_0.02415rem_0.24155rem_rgba(0,0,0,0.4)]';
      }
    } else if (isOnLoginPage) {
      currentHeaderStyle.background = 'linear-gradient(315deg, #eb4eb1, #9257f9 50.46%, #3150ed)';
    } else { 
  ...
  // Specific override for Blog Page background if not already white (e.g., if mobile logic changed it)
  // And ensure border is present at top for blog page, even if other conditions tried to remove shadow-none
  if (isOnBlogPage) {
    currentHeaderStyle.background = 'white'; // Always white for blog page
    if (isAtTop && !headerContainerClasses.includes('border-b')) { // Ensure border if at top and somehow removed
       headerContainerClasses = headerContainerClasses.replace(/\bshadow-lg\b|\bshadow-none\b/g, '').trim() + ' shadow-none border-b border-gray-300';
    } else if (isAtTop) {
      // Ensure shadow-none is present if border is also present
      if (!headerContainerClasses.includes('shadow-none')) {
         headerContainerClasses = headerContainerClasses.replace(/\bshadow-lg\b/g, '').trim() + ' shadow-none';
      }
    }
  }


  headerContainerClasses += ` ${headerIsNudgingUp ? '-translate-y-full' : 'translate-y-0'}`;
        ...
        <div className="main-container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-full max-w-container-wide">
            ...
            <nav className="hidden md:flex items-center space-x-12 lg:space-x-16 xl:space-x-20 justify-end">
              {mainNavLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) => {
                    const baseClasses = 'text-[1.6rem] lg:text-[1.8rem] font-medium transition-colors duration-150';
                    let colorClasses = '';

                    if (isActive) {
                      if (useLightTextAndLogo && !isOnBlogPage) {
                        colorClasses = 'text-white';
                      } else {
                        colorClasses = 'text-brand-purple';
                      }
                    } else {
                      if (!isMobileView && (isOnHomePage || isOnReferralPage) && !isOnBlogPage) {
                        colorClasses = 'text-[rgba(255,255,255,.7)] hover:text-white';
                      } else if (useLightTextAndLogo && !isOnBlogPage) {
                        colorClasses = 'text-gray-200 hover:text-white';
                      } else {
                        colorClasses = 'text-brand-text-muted hover:text-brand-purple';
                      }
                    }
              ...
              >
                <button
                  ref={qrButtonRef} 
                  className={`text-[1.6rem] lg:text-[1.8rem] font-medium transition-colors duration-150 cursor-default ${
                    isQrPopoverOpen && window.innerWidth >=768 
                      ? (useLightTextAndLogo && !isOnBlogPage)
                        ? 'text-white'
                        : 'text-brand-purple'
                      : (() => {
                          if (!isMobileView && (isOnHomePage || isOnReferralPage) && !isOnBlogPage) {
                            return 'text-[rgba(255,255,255,.7)] hover:text-white';
                          } else if (useLightTextAndLogo && !isOnBlogPage) {
                            return 'text-gray-200 hover:text-white';
                          } else {
                            return 'text-brand-text-muted hover:text-brand-purple';
                          }
                        })()
                  }`}
                  aria-haspopup="true"
              ...
              <div className="flex items-center">
                <Link
                  to="/login"
                  className={`flex items-center justify-center text-[1.6rem] lg:text-[1.8rem] font-medium px-8 h-[4.4rem] rounded-full transition-all duration-300 transform hover:scale-105
                    ${
                      (useLightTextAndLogo && !isOnLoginPage && !isOnBlogPage)
                ...
                >
                  <PersonIcon
                    className="w-[22px] h-[22px] mr-2 text-white"
                  />
          ...
          <div className="md:hidden flex items-center">
            {isOnLoginPage ? (
              ...
              >
                <NewMenuIcon className="w-[2.3rem] h-[2rem]" aria-hidden="true" />
              </button>
            )}
          </div>
        </div>
