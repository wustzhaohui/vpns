...
const Footer: React.FC = () => {
  ...
  const contactIcons: ContactIconItem[] = [
    {
      name: 'Email',
      icon: <img src="/assets/email.2e4e9014.svg" alt="Email us" className="w-[2rem] h-[2rem] sm:w-[2.5rem] sm:h-[2.5rem] md:w-[3rem] md:h-[3rem]" />,
      href: 'mailto:support@example.com',
      label: 'Email us',
    },
    { 
      name: 'X', 
      icon: <img src="/assets/twitter.9499bc01.svg" alt="Follow us on X" className="w-[2rem] h-[2rem] sm:w-[2.5rem] sm:h-[2.5rem] md:w-[3rem] md:h-[3rem]" />,
      href: '#', 
      label: 'Follow us on X' 
    },
    {
      name: 'Instagram',
      icon: <img src="/assets/ins.6c93479c.svg" alt="Follow us on Instagram" className="w-[2rem] h-[2rem] sm:w-[2.5rem] sm:h-[2.5rem] md:w-[3rem] md:h-[3rem]" />,
      href: '#',
      label: 'Follow us on Instagram',
    },
    { 
      name: 'Telegram', 
      icon: <img src="/assets/telegram.3351da03.svg" alt="Join us on Telegram" className="w-[2rem] h-[2rem] sm:w-[2.5rem] sm:h-[2.5rem] md:w-[3rem] md:h-[3rem]" />,
      href: '#', 
      ...
      <div className="main-container mx-auto px-4 sm:px-6 lg:px-8 max-w-container-wide py-8 sm:py-10 md:py-12 lg:py-16 border-t border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-y-10 md:gap-8">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-8 md:contents">
            <div>
              <h3 className={`${footerSectionTitleFontSize} font-semibold text-brand-text-primary mb-4 sm:mb-5 md:mb-[25px] lg:mb-[30px]`}>
              ...
              <ul className="space-y-3 sm:space-y-4 md:space-y-[1.8rem] lg:space-y-[2rem]">
                {softwareDownloads.map((link) => (
                  <li key={link.nameKey}>
                    <Link
                      to={link.href}
                      className={`${footerLinkFontSize} text-gray-600 hover:text-[#747E8B] transition-colors duration-200`}
                    >
                      {t(link.nameKey)}
                    </Link>
                  </li>
            ...
            <div className="relative" ref={languageDropdownRef}>
              ...
              <button
                id="language-button"
                className={`${footerLinkFontSize} text-gray-600 hover:text-[#747E8B] transition-colors duration-200 flex items-center`}
              ...
              >
                ...
                <svg
                  ...
                  className={`h-4 w-4 ml-1 transition-transform duration-200 ${isLanguageDropdownOpen ? 'transform rotate-180' : ''}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              {isLanguageDropdownOpen && (
                <div 
                  className="absolute left-0 bottom-full mb-2 w-max bg-white rounded-md shadow-lg border border-gray-200 py-1 z-10" // Changed to open upwards
                  role="menu"
                ...
                >
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 ${footerLinkFontSize} ${i18n.language.startsWith('zh') && !i18n.language.startsWith('zh-Hant') ? 'font-semibold text-brand-purple' : 'text-gray-700'} hover:bg-gray-100 hover:text-brand-purple`}
                    role="menuitem"
                  >
                    简体中文
                  </button>
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 ${footerLinkFontSize} ${i18n.language.startsWith('en') ? 'font-semibold text-brand-purple' : 'text-gray-700'} hover:bg-gray-100 hover:text-brand-purple`}
                    role="menuitem"
                  >
                    English
                  </button>
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 ${footerLinkFontSize} ${i18n.language.startsWith('zh-Hant') ? 'font-semibold text-brand-purple' : 'text-gray-700'} hover:bg-gray-100 hover:text-brand-purple`}
                    role="menuitem"
                  >
                    繁體中文
                  </button>
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 ${footerLinkFontSize} ${i18n.language.startsWith('ru') ? 'font-semibold text-brand-purple' : 'text-gray-700'} hover:bg-gray-100 hover:text-brand-purple`}
                    role="menuitem"
                  >
                    Русский
                  </button>
