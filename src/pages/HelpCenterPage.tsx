...
const HelpCenterChevronDownIcon: React.FC<React.SVGProps<SVGSVGElement>> = (
  props
) => (
  <svg
    fill="none"
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {isMobileMenuOpen && (
        ...
        >
          ...
          ></div>
          <div
            className="fixed top-4 right-4 w-[calc(100%-3rem)] max-w-[280px] max-h-[calc(100vh-2rem)] bg-header-gradient-middle/95 backdrop-blur-sm rounded-xl shadow-2xl flex flex-col text-white"
            style={{ background: 'var(--header-gradient-middle, #8936EB)' }}
          >
            <div className="flex justify-between items-center p-3 border-b border-white/20">
              ...
              >
                {t('helpCenterPage.internalHeader.menuTitle')}
              </span>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-2"
            ...
            <nav className="flex flex-col px-4 py-3 space-y-1 overflow-y-auto">
              <a
                href="https://letsvpn.world/"
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setIsMobileMenuOpen(false)}
                className="block py-3 text-lg hover:bg-white/10 rounded px-2 transition-colors"
              >
                {t('helpCenterPage.internalHeader.goToLetsVPN')}
              </a>
              <Link
              ...
              <div
                className="flex items-center py-3 text-lg text-gray-300 px-2 cursor-default"
                aria-label={t(
                  'helpCenterPage.internalHeader.languageAriaLabel'
                )}
              >
